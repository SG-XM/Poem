...
public class MeaningFragment extends LazyFragment {
        ...
        generate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!CharacterUtil.checkCharacter(editText.getText().toString()))
                    Toast.makeText(MeaningFragment.this.getContext(), "请输入中文", Toast.LENGTH_SHORT).show();
                else if (editText.toString().length() < 4)
                    Toast.makeText(MeaningFragment.this.getContext(), "种子不能少于四个字", Toast.LENGTH_SHORT).show();
                else
                    MainViewModel.getInstance().write("/poem", ModelApi.TYPE.MEANING, editText.getText().toString(), 0);

            }
        });

        MainViewModel.getInstance().getMeaningData().observe(this, new Observer<List<String>>() {
            @Override
